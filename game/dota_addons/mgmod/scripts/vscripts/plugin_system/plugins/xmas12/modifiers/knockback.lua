...
--------------------------------------------------------------------------------
-- Classifications
function knockback:IsHidden()
	return true
end

function knockback:IsPurgable()
	return false
end

function knockback:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
--------------------------------------------------------------------------------
-- Initializations
function knockback:OnCreated( kv )
	if IsServer() then
...
function knockback:OnRefresh( kv )
	if not IsServer() then return end
end

function knockback:OnDestroy( kv )
	if not IsServer() then return end
...
--------------------------------------------------------------------------------
-- Setter
function knockback:SetEndCallback( func ) 
	self.EndCallback = func
end

--------------------------------------------------------------------------------
-- Modifier Effects
function knockback:DeclareFunctions()
	local funcs = {
...
function knockback:GetOverrideAnimation( params )
	if self.anim==1 then
...
--------------------------------------------------------------------------------
-- Motion effects
function knockback:UpdateHorizontalMotion( me, dt )
...
function knockback:OnHorizontalMotionInterrupted()
	if IsServer() then
...
function knockback:UpdateVerticalMotion( me, dt )
	-- set time
...
function knockback:OnVerticalMotionInterrupted()
	if IsServer() then
		self.interrupted = true
		self:Destroy()
	end
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function knockback:GetEffectName()
	if not IsServer() then return end
	if self.stun then
...
function knockback:GetEffectAttachType()
	if not IsServer() then return end
